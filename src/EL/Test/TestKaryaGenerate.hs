...
main = do
    ...
    (origFile, inputFile, outputFile) <- case args of
        [origFile, _inputFile, outputFile] ->
            return (origFile, _inputFile, outputFile)
        _ -> error "expected: origFile inputFile outputFile"
    inputFiles <- find (Char.isUpper . head) (".hs" `List.isSuffixOf`)
        (FilePath.takeDirectory origFile)
    extracted <- ExtractHs.extractFiles extract inputFiles
    let (warnings, output) = generate outputFile extracted
